...
/********************************************
    ...
    此部分代码对NRF24L01进行初始化
		无线收到的数据说明如下：
    无线收到的为*位的字符串  eg:ffxxyywwff
    其中前后各两位的ff为规定的数据格式
    xx表示x方向的速度，单位为m/s,第一个x表示整数位，第二个x表示小数位
    yy表示y方向的速度，单位为m/s,第一个y表示整数位，第二个y表示小数位
    ww表示y方向的速度，单位为rad/s,第一个w表示整数位，第二个w表示小数位

********************************************/



u8 tmp_buf[13];	                                                                      //定义此数组用于存放无线的数据
int t;                                                                                //无话可说
...
{  
  ...
  GPIO_Init(GPIOG, &GPIO_InitStructure);                                              //初始化GPIOG6,GPIOG7
...
/**************NRF24L01检测函数，若其不正常，则LED0快闪*****************/
	...
	NRF24L01_RX_Mode();		                                                              //NRF24L01初始化为接收模式
...
{ 
	   int t;
     if(NRF24L01_RxPacket(tmp_buf)==0)                                                 //判断是否接收到消息
			{
				for(t=0;t<13;t++)
				//    printf("%c",tmp_buf[t]);
			 //  printf("\r\n");
					basketballbot.robot_expected_speed.x=(tmp_buf[3]-48)+(tmp_buf[4]-48)*0.1;    //计算机器人自身目标x速度
				  if(tmp_buf[2]=='1') basketballbot.robot_expected_speed.x=-basketballbot.robot_expected_speed.x;	
				  basketballbot.robot_expected_speed.y=(tmp_buf[6]-48)+(tmp_buf[7]-48)*0.1;    //计算机器人自身目标y速度
				  if(tmp_buf[5]=='1') basketballbot.robot_expected_speed.y=-basketballbot.robot_expected_speed.y;	
				  basketballbot.robot_expected_speed.w=(tmp_buf[9]-48)+(tmp_buf[10]-48)*0.1;    //计算机器人自身目标w速度
	       ...
	       // Set_Speed_Robot_To_Motor();                                                  //机器人自身目标速度转换为电机速度
	       // DRIVER_Set_Speed();                                                          //驱动盒发送电机目标速度给电机
			}
			else ;
